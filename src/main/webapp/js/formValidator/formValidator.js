...
function checkCharLength(fontSize,obj){
	if(typeof(obj)=="string"){
		var tt = $("#"+obj).val();
		if(tt.length>fontSize){
			var value = tt.substr(0,fontSize);
			$("#charLen"+obj).text(value.length+'/'+fontSize);
			$("#"+obj).val(value);
		}else{
			$("#charLen"+obj).text(tt.length+'/'+fontSize);
		}
	}else{
		var tt = obj.val();
		var id = obj.attr('id');
		var value = tt.substr(0,fontSize);
		var inpitLen = value.length;
		//var inpitLen = value.replace(/[^\x00-\xff]/g, "**").length;
		$("#charLen"+id).text(inpitLen+'/'+fontSize);
		obj.val(value);
	}
}
