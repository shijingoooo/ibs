...
/**
 * 
 * @ClassName: PageBeanAjax
 * @Description: 分页显示的标准类,基本操作,是先给予-当前页数一共的数据条数-每页显示的条数,
 *               然后在初始化该类,得到总共页数,和开始序号和结束序号,
 *               然后数据库分页用到开始序号和结束序号,得到数据集合后赋值给该类的list属性, 然后把该类发送到jsp页面,进行访问
 * @author 李华洞
 * @date 2016年8月10日 上午10:57:27
 * @version 1.0
 * @param <T>
 */
public class PageBeanAjax<T>
{
    ...
    public String getHtml()
    {
        ...
        // sb.append(" function goToPageAjax" + this.form
        // + "(action, form_id, index, flg) { ");
        // sb.append("\r");
        // sb.append("   refresh(); ");
        // sb.append("\r");
        ...
        {
            sb.append("     <a onclick=\"goToPageAjax"
                    + this.form
                    + "('"
                    + "0"
        ...
        if (totalPages > 1)
        {
            ...
            sb.append("     onclick=\"goToPageAjax" + this.form + "("
                    + "'0','flg');\">跳转</button>     ");
        }
        else
        {
    ...
    public void createNavi(long num)
    {
        ...
        for (long i = startIndex; i <= total; i++, breakFlg++)
        {
            ...
            if (i == curentPage)
                ...
                navi.append("<a onclick=\"goToPageAjax"
                        + this.form
                        + "('"
                        + (i - 1)
                        + "',''); \" href=\"javascript:void(0);\" class=\"navi\">"
                        + i + "</a>&nbsp;");
        }
